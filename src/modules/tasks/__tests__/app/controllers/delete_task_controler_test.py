...
class TestDeleteTaskController:
    ...
    async def test_execute_validate_body(self):
        arrange = [
            {"body": {}, "message": ["id: Field required"]},
            {"body": {"id": 1}, "message": ["id: Input should be a valid string"]},
            {"body": {"id": True}, "message": ["id: Input should be a valid string"]},
        ]

        for item in arrange:
            response = await self.controller.execute({"body": item["body"]})
    ...
    async def test_execute_return_error_usecase(self):
        ...
        response = await self.controller.execute({"body": {"id": "test"}})
